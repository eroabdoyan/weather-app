      ...
      try {
        if(inputLocation === '') {
          if (location.lat !== null && location.lon !== null) {
            const currentWeatherResponse = await axios.get(`http://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${location.lat},${location.lon}`);
            setCurrentWeather(currentWeatherResponse.data);

            const forecastResponse = await axios.get(`http://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${location.lat},${location.lon}&days=7`);
          ...
          const currentWeatherResponse = await axios.get(`http://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${inputLocation}`);
          setCurrentWeather(currentWeatherResponse.data);

          const forecastResponse = await axios.get(`http://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${inputLocation}&days=7`);
          setDailyForecast(forecastResponse.data.forecast.forecastday)
          setHourlyForecast(forecastResponse.data.forecast.forecastday[0].hour);
        }
